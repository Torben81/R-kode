...
init <- function(d,al=.01){#d: dataframe al:level of significance
    ...
    eli <- matrix(F,L,L)#info about each edge. Is it eligable for
                        #adding or removal?
    crit <- qnorm(1-al/2)
    for(i in 1:L) eli[i,vL[[i]]$t>crit] <- T
...
oneStep <- function(m){
    ...
    #either eligable for add=T or remove.
    eli <- (1:m$L^2)[m$eli]
